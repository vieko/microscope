...
ClickedPostsController = PostsListController.extend({
  ...
  nextPath: function () {
    return Router.routes.clickedPosts.path({postLimit: this.postsLimit() + this.increment});
  }
});
...
Router.route('/feed.xml', {
  ...
  action: function () {
    var feed = new RSS({
      title: 'New Skystone Links',
      description: 'the latest posts from Skystone Links, a link aggregator.'
    });
